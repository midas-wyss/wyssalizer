...
extract_papers <- function(search_term) {
  a1 <- EUtilsGet(EUtilsSummary(paste0(search_term, "[au]"), type = "esearch", db = "pubmed"))
  a2 <- EUtilsGet(EUtilsSummary(paste0(search_term, "[au] AND ", "wyss institute[ad]"), type = "esearch", db = "pubmed"))
  return(wyss_papers)
}
...
data_extractor <- function(papers, faculty, start_year) {
  ...
  x1 <- Affiliation(a1)
  aff <- vector(mode = "list", length = length(x1))
  for (i in seq(1, length(x1))) {
    aff[[i]] <- tibble::tibble(pmid = pmids[i],
                               affiliation = as.tibble(x1[i]) %>% as.matrix() %>% as.vector())
  }
  aff <- dplyr::bind_rows(aff)
  
  y1 <- which(year_pubmed >= start_year)
  # y2 <- grep("Boston", aff$affiliation)
  # subs <- which(pmids[y1] %in% aff$pmid[y2])
